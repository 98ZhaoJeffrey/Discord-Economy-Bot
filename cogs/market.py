...
class Market(commands.Cog):
    ...
    async def buy(self, ctx, id: int, quantity: int):
        user = ctx.message.author
        try:
            dbUser = userbridge.Get(user).to_dict()
    ...
    @commands.command()
    #returns the items the user has or queries for the items that contains the keyword
    async def market(self, ctx, argument: String = None):
        user = ctx.message.author
        if(String):
            try:
                ...
                message = discord.Embed(
                    title = f"Items sold by {dbItem.owner}",
